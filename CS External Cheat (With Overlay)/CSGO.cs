...
namespace CSGO_S
{
    ...
    public class CSGO : AdvancedProcess
    {
        ...
        public CSGO(int updateInterval) : base("csgo.exe")
        {
        ...
        private void Initialize()
        {
            ...
            Overlay.WaitForConnectAsync(3000).Wait();
        ...
        public class Player
        {
            ...
            public Player(CSGO csgo, string address)
            {
                this.csgo = csgo;
            ...
            {
                // Is Exists
                MemoryAddress = csgo.Memory.ReadPointer(pointer, false)!;
                IsExists = MemoryAddress != UIntPtr.Zero;
                if (!IsExists) return;

                Team = (TeamType)csgo.Memory.Read<int>($"{MemoryAddress}+{m_iTeamNum}", false);

                IsRendered = !csgo.Memory.Read<bool>($"{MemoryAddress}+{m_bDormant}", false);
                if (!IsRendered) return;

                LifeState = !csgo.Memory.Read<bool>($"{MemoryAddress}+{m_lifeState}", false);

                IsAlive = LifeState && (Team == TeamType.Terrorists || Team == TeamType.CounterTerrorists);
                if (!IsAlive) return;

                Health = csgo.Memory.Read<int>($"{MemoryAddress}+{m_iHealth}", false);
                Armor = csgo.Memory.Read<int>($"{MemoryAddress}+{m_ArmorValue}", false);

                byte[] coordinates = csgo.Memory.ReadBytes($"{MemoryAddress}+{m_vecOrigin}", 12, false)!;
            ...
            {
                var viewMatrix = csgo.ViewMatrix;
        ...
        public class Matrix
        {
            ...
            private CSGO csgo;
            private string pointer;
            public Matrix(CSGO csgo, string address)
            {
                this.csgo = csgo;
                this.pointer = address;
            }
            ...
            public void Update()
            {
                byte[] viewMatrixData = csgo.Memory.ReadBytes(pointer, 16 * 4, false)!;
